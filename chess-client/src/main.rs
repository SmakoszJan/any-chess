...
use chess_core::{Board, ChessMove, Kind, Move, Piece};

use crate::ui::{HideUi, Promote, ShowUi};
...
fn on_stage_move(
    ...
    if let MoveState::Start(current) = *state
    ...
    *state = ev.0.map_or(MoveState::None, MoveState::Start);
...
fn on_promote(
    ...
    let MoveState::Promote { from, to } = *state else {
        return;
    };
    let from = pieces.get(from).unwrap();
    let to = pieces.get(to).unwrap();

    commands.trigger(MakeMove(ChessMove {
...
fn main() {
    App::new()
        .add_plugins((DefaultPlugins, MeshPickingPlugin, ui::plugin))
        .add_systems(Startup, setup)
        .add_systems(Update, (sync_board,))
        .insert_resource(ChessBoard {
            board: Board::new(),
        })
        .insert_resource(SelectedSquare(None))
        .insert_resource(MoveState::None)
